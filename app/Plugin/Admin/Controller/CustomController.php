...
class CustomController extends AdminAppController {
	...
	public function index() {
		...
		$conditions = array("Product.type" => "custom");

		if (!empty($start) && !empty($end)) {
			$duration = array("AND" => array( array("Product.created >=" => strtotime($start)), array("Product.created <=" => strtotime($end))));
			$conditions = array_merge($conditions, $duration);
		}

		if (!empty($keyword)) {
			$search = array("OR" => array( array("Product.name LIKE " => "%" . $keyword . "%"), array("Product.description LIKE " => "%" . $keyword . "%")));
		...
		if (!empty($categories)) {
			...
			$data = $this -> Product -> find('all', array('joins' => array( array('table' => 'category_to_object', 'alias' => 'CategoryToObject', 'type' => 'inner', 'foreignKey' => false, 'conditions' => array('CategoryToObject.object_guid = Product.guid')), ), 'conditions' => $conditions, 'order' => 'modified DESC', 'limit' => $limit, 'page' => $page + 1, 'fields' => array("Product.*")));

			//$this->Product->_debug ();

			$total = $this -> Product -> find('count', array('joins' => array( array('table' => 'category_to_object', 'alias' => 'CategoryToObject', 'type' => 'inner', 'foreignKey' => false, 'conditions' => array('CategoryToObject.object_guid = Product.guid')), ), 'conditions' => $conditions));
			...
			if ($total <= 0) {
				...
				$data = $this -> Product -> find('all', array('limit' => $limit, 'page' => $page + 1, 'conditions' => $conditions, 'order' => 'modified DESC', 'fields' => array("Product.*")));
			}
		}

		foreach ($data as $key => $value) {

			if ($value['Product']['type'] == 'custom') {
		...
		$header = array("name" => "Name", "type" => "Type", "image" => "Picture", "price" => "Price", "quantity" => "Quantity", "created" => "Created", "modified" => "Modified");

		$this -> set(array("header" => $header, "data" => $data, "page" => $page, "limit" => $limit, "pages" => $pages, "keyword" => $keyword, "start" => $start, "end" => $end, "filter" => $filter, "filters" => $filters, "filter_categories" => empty($filter_categories) ? $filter_categories : json_encode($filter_categories), ));
	}
	...
	public function add() {
		...
		if ($this -> request -> is('ajax') && $this -> request -> is('post')) {
			...
			if ($is_special == 1) {
				...
				$special = array("is_special" => 1, "special_price" => $data['special_price'], "special_start" => strtotime($data['special_start']), "special_end" => strtotime($data['special_end']));
			...
			if (!empty($category)) {
				...
				foreach ($category as $value) {
					$c[] = array("category_guid" => $value, "object_guid" => $data['guid']);
				}
	...
	public function categoryfilter() {
		...
		if ($level == 0) {
			...
			$conditions = array('level' => $level, 'parent_guid' => $guid);
		}

		$data = $this -> Category -> find('all', array("conditions" => $conditions, "order" => array('order ASC')));
	...
	protected function _categoryList($data, &$return) {
		if (!empty($data)) {
			foreach ($data as $value) {
				...
				$result = $this -> Category -> find('all', array("conditions" => array('parent_guid' => $value['Category']['guid']), "order" => array('order ASC')));
	...
	public function install() {
		...
		$templates = array("iphone5" => array("name" => "iphone5", "description" => "iphone5 case", "price" => "34.99", "image" => array("foreground" => "iphone5_fg.png", "background" => "iphone5_bg.png", ), "type" => "template", "status" => "published", "quantity" => 65535, "order" => 0, ), "iphone4" => array("name" => "iphone4", "description" => "iphone4 case", "price" => "34.99", "image" => array("foreground" => "iphone4_fg.png", "background" => "iphone4_bg.png", ), "type" => "template", "status" => "published", "quantity" => 65535, "order" => 1), "samsung galaxy 3" => array("name" => "samsung galaxy 3", "description" => "iphone5 case", "price" => "34.99", "image" => array("foreground" => "samsung galaxy 3-outer.png", "background" => "samsung galaxy 3-inner.png", ), "type" => "template", "status" => "published", "quantity" => 65535, "order" => 2, ), "samsung galaxy 4" => array("name" => "samsung galaxy 4", "description" => "samsung galaxy 4", "price" => "34.99", "image" => array("foreground" => "samsung galaxy 4-outer.png", "background" => "samsung galaxy 4-inner.png", ), "type" => "template", "status" => "published", "quantity" => 65535, "order" => 3), );
