...
( function($) {
		...
		var drawing = {
			...
			// @board : refered to canvas and itss global variables
			board : {
				...
				canvas : canvas,
				canvasScale : 0,
				originWidth : 780, // original width
				originHeight : 780, // original height
				originX : 0, // canvas postion - startup - fixed
				originY : 0, // canvas postion - startup - fixed
				left : 0, // canvas postion - dynamically
				top : 0, // canvas postion - dynamically
				offset : 0 // canvas postion, mouse position are different
			...
			// @content : store different pages and its index
			content : {
				pages : [],
				index : 0
			},
			
			// @grid : store grid system 
			grid : {
				lines : null,
				size : 20,
				visible : false,
		...
		var Tools = function () {
			...
			this.backgroundimage = function(url) {
			...
			this.overlayimage = function(url) {
			...
			this.reload = function(json, callback) {
				this.zoomreset();
				canvas.clear();
		...
		Tools.prototype = drawing;
		
		var TextEditor = function () {
			this.containerId = ".text-editor",
			this.id = null,
			this.current = null;
			...
			this.changeFontProperty = function(property) {
			...
			this.changeFontColor = function(color) {
			...
			this.changeFontAlign = function(align) {
		...
		var imageEditor = $.extend (drawing, {
		...
		var mememaker = $.extend (drawing, {
			...
			// methods
			init : function(id, backgroundcolor, scale) {
				this.board.canvasId = id;
				this.board.canvasScale = scale;
	
				this.board.canvas = canvas = new fabric.Canvas(this.canvasId);
	
				this.width = canvas.getWidth();
				this.height = canvas.getHeight();
	
				canvas.backgroundColor = this.board.backgroundColor = backgroundcolor;
				...
				this.board.originX = this.board.left;
				this.board.originY = this.board.top;
	
				localStorage.pages = JSON.stringify(this.content.pages);
